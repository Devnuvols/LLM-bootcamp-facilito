...
def parse_doc(docx_path:str) -> List[Dict[str, str]]:
    """
    Return a list of dictionaries containing text with metadata.
    Dictionary:
        name -> Document name
    ...
    pages_text = []
    for k, paragraph in enumerate(doc.paragraphs):
        new = {
            'name': docx_path,
            'page': k + 1,
...
def parse_pptx(pptx_path:str) -> List[Dict[str, str]]:
    """
        Return a list of dictionaries containing text with metadata.
        Dictionary:
        name -> Document name
        ...
        content -> shape text
    """
    print(f"Parsing pptx file {pptx_path}")
...
def process_file(pdf_path: str, method: str) -> List[Dict[str, Any]]:
    ...
    """
    file_extension = os.path.splitext(pdf_path)

    if file_extension[1] == '.pdf':
        pages_text = parse_pdf(pdf_path)
    elif file_extension[1] == '.docx':
        pages_text = parse_doc(pdf_path)
    elif file_extension[1] == '.pptx':
        pages_text = parse_pptx(pdf_path)
...
def truncated_string(string: str, max_tokens: int, print_warning: bool = True) -> str:
...
def split_strings(string: str, max_tokens: int = 1000, max_recursion: int = 5) -> list[str]:
    """
